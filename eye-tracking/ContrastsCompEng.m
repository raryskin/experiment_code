...
if blocknum == 1
    ...
    %% reads in trial list
     TrialList = readtable(['ContrastsComp_L', num2str(counterbalancelist), '-1.txt']); 
...
%% Spawn a second Matlab session2 that records GP3 data to output file
   ...
   % FAVOR OF RUNNING THE CALIB DIRECTLY THROUGH GAZEPOINT - 08/20/2018
...
name_of_file=['output/Contrasts_S' subject '_b' num2str(blocknum) '_' srand '.csv'];
